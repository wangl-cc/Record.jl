...
@testset "show: dynamic" begin
    ...
    @test stringshow(S) ==
        "recorded 1.0"
    @test stringshow(V) ==
        "recorded 1-element Vector{Float64}:\n 1.0"
...
@testset "show: test" begin
    ...
    @test stringshow(V) ==
        "recorded 1-element Vector{Float64}:\n 1.0"
    @test stringshow(M) ==
        "recorded 1×1 Matrix{Float64}:\n 1.0"
    @test stringshow(record(S)) ==
        "record for 0-dimensional Float64 with time Int64"
    @test stringshow(record(V)) ==
        "record for 1-element Vector{Float64} with time Int64"
    @test stringshow(record(M)) ==
        "record for 1×1 Matrix{Float64} with time Int64"
end
